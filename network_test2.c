...
int main(int argc,char **argv)
{
    ...
    */
	...
	if(test_parameters.test_type==NEW_TEST_TYPE)
        {
*/	    void *library_handler;
	//    int (*new_test)(int mes_length);
	    int (*test)(Test_time_result_type *times,int mes_length, int num_repeats, int num_noise_repeats, int noise_message_length, int num_noise_procs);
	    char *adress=NULL; 
	    get_test_type_name(test_parameters.test_type,test_type_name);
	    ...
	    strcat(adress,"2.so");
	    printf("%s\n",adress);
	    library_handler =dlopen(adress,RTLD_LAZY);
	    if(!library_handler)
	    {
